...
class NmChainClient:
    ...
    def from_env(cls) -> Optional["NmChainClient"]:
        base_url = _env_first("BILLING_CHAIN_API_BASE", "NMCHAIN_API_BASE")
        if not base_url:
            return None
        timeout_raw = _env_first("BILLING_CHAIN_TIMEOUT", "NMCHAIN_TIMEOUT", default="10")
        try:
        ...
        return cls(
            base_url,
            app_id=_env_first("BILLING_CHAIN_APP_ID", "NMCHAIN_APP_ID", default="billing"),
            api_token=_env_first("BILLING_CHAIN_API_TOKEN", "NMCHAIN_API_TOKEN"),
            timeout=timeout,
        )
    ...
    def list_blocks(self, *, limit: int = 20) -> Dict[str, Any]:
        limit_val = max(1, min(int(limit), 200))
        return self._request("GET", "/api/blocks", params={"limit": limit_val})
